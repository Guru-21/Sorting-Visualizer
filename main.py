...
def StartSortingAlgorithm():
    ...
    elif algorithmList.get() == 'Selection Sort':
        SelectionSort(data, drawDataArray, sortingSpeedScale.get())

    elif algorithmList.get() == "Shell Sort":
        ShellSort(data, drawDataArray, sortingSpeedScale.get())
...
UserInterfaceFrame = Frame(root, width=1400, height=200, bg='yellow')
...
canvas = Canvas(root, width=1400, height=550, bg='pink')
...
algorithmList = ttk.Combobox(UserInterfaceFrame, textvariable=selectedAlgorithm,
                             values=['Bubble Sort', 'Quick Sort', 'Merge Sort', 'Insertion Sort','Selection Sort','Shell Sort'])
